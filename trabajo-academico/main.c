...
void graphPrintMetrics(TGraph* g, FILE* fp, int type, int nthreads, int nerdos, int nrep, int proption, int exp) {
	...
	clock_t start;
	clock_t end;
	/* En Windows, se puede hallar el tiempo real con alta precisión */
	LARGE_INTEGER wfrequency;
	LARGE_INTEGER wstart;
	LARGE_INTEGER wend;
	double wseconds = 0;
	QueryPerformanceFrequency(&wfrequency);
	...
	for (i = 0; i < nrep; ++i) {
		...
		wseconds += (double)(wend.QuadPart - wstart.QuadPart) / wfrequency.QuadPart;
	}
	seconds = seconds / nrep;
	wseconds = wseconds / nrep;
	...
	fprintf(fp, "Time: %.5lf s, Wallclock: %.6lf s\n", seconds, wseconds);
	fprintf(fp, "____________________________\n");
	...
	fprintf(fp, "Time: %.5lf s, Wallclock: %.6lf s, Iterations: %d, Total: %.14lf\n", seconds, wseconds, it, totalpr);
	fprintf(fp, "____________________________\n");
